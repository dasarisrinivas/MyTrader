...
def load_bot_state(
    ...
    Returns:
        (consecutive_loss_count, extra_cooldown_until)

    Handles:
      - Missing file → returns (0, None)
      - Corrupt JSON → returns (0, None) with warning
      - Day rollover → resets consecutive_loss_count to 0
    ...
    # ── Day rollover: reset loss counter if last trade was on a different CT date ──
    ...
    return loss_count, cooldown_until
