...
from .order_lock import OrderLockManager, SubmissionDeduplicator
...
class TradeExecutor:
    ...
    def __init__(
        ...
        self._dedup = SubmissionDeduplicator(
            symbol=symbol,
            ttl_seconds=getattr(config, "idempotency_signature_ttl_seconds", 900),
            order_tracker=self.order_tracker,
        )
        # Backward-compat aliases (used by _place_order / _on_execution)
        self._submission_signatures = self._dedup._signatures
        self._signature_ttl_seconds = self._dedup._ttl
        self._signal_submission_keys = self._dedup._signal_keys
        self._signal_ttl_seconds = self._dedup._ttl
        ...
        self._lock = OrderLockManager(
            timeout_seconds=getattr(config, "order_lock_timeout_seconds", 300),
            cancel_trade_fn=self._cancel_trade,
    ...
    # =========================================================================
    # Submission dedup — delegates to SubmissionDeduplicator
    # =========================================================================
    ...
    def generate_submission_signature(
        ...
        return self._dedup.generate_signature(action, quantity, metadata)

    def is_duplicate_submission(self, signature: str) -> bool:
        """Check if a submission signature is a duplicate."""
        return self._dedup.is_duplicate(signature)
    ...
    def record_submission(
        ...
        self._dedup.record(signature, action, quantity, metadata)
    ...
    def _build_signal_submission_key(self, action: str, metadata: Dict[str, Any]) -> str:
        ...
        return self._dedup.build_signal_key(action, metadata)

    def _is_duplicate_signal_key(self, key: str) -> bool:
        return self._dedup.is_duplicate_signal_key(key)

    def _record_signal_key(self, key: str) -> None:
        self._dedup.record_signal_key(key)
    ...
    async def place_order(
        ...
        if self._lock.is_locked:
            lock_age = self._lock.age_seconds
            logger.warning(
                f"Skipping trade; order lock active for {lock_age:.1f} seconds (reason: {self._lock.reason})"
            )
            from ib_insync import Trade as IBTrade
            dummy_trade = IBTrade()
        ...
        if metadata:
            order.orderRef = f"Shree_{metadata.get('signal_source', 'manual')}"
    ...
    def get_active_order_count(self, sync: bool = False) -> int:
        ...
        if sync:
            ...
            logger.info(
                ...
                lock_state=(
                    f"{self._lock.reason} {self._lock.age_seconds:.0f}s"
                    if self._lock.is_locked
                    else "idle"
                ),
            )
            ...
            if not synced_orders and self._lock.is_locked:
