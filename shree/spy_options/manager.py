...
class SpyOptionsManager:
    ...
    async def _poll(self, spy_conid: int) -> None:
        ...
        self._last_max_pain: Optional[float] = None        # will be set after chain build
        self._last_near_max_pain: bool = False
    ...
    def _format(self, sig: SpySignal) -> str:
        ...
        # Show the most actionable levels so the trader can see the full picture
        ext_obj = context.external if hasattr(sig, "_ext_obj") else None
        # We pull these from the ExternalContext that was passed into SignalContext;
        # since _format() only receives sig, we re-read from the manager's last context.
        # Use signal reasoning as a proxy — levels were already embedded there.
        # Instead, show what was captured in the ORB / VWAP / pivot fields if available:
