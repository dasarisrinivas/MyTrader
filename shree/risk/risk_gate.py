...
class RiskGateConfig:
    ...
    weekly_max_loss_usd: float = field(default_factory=lambda: float(os.environ.get("WEEKLY_MAX_LOSS_USD", "500")))
...
class RiskGate:
    ...
    def __init__(self, config: RiskGateConfig):
        ...
        # FEB 2026: Weekly P&L tracking
        self._weekly_realized_pnl: float = 0.0
        self._weekly_reset_date: Optional[datetime] = None  # Monday of current tracking week
        # FEB 2026: Kill-switch
        self._kill_switch_active: bool = False
        self._kill_switch_trigger_ts: Optional[datetime] = None
        self._kill_switch_manual: bool = False  # Manual kill requires manual reset
        self._daily_limit_hit_count_this_week: int = 0
    ...
    def _ensure_weekly_reset(self, now: Optional[datetime] = None) -> None:
        """Auto-reset weekly P&L on Monday RTH open (09:30 ET)."""
        ts = now or now_cst()
        try:
            # Convert to CT for day-of-week check
            cst_now = ts.astimezone(CST) if ts.tzinfo else now_cst()
        except Exception:
            cst_now = now_cst()

        # Find Monday of the current week
        days_since_monday = cst_now.weekday()  # Mon=0
        current_monday = cst_now.date() - timedelta(days=days_since_monday)

        if self._weekly_reset_date is None or current_monday > self._weekly_reset_date.date():
            prev_pnl = self._weekly_realized_pnl
            self._weekly_realized_pnl = 0.0
            self._weekly_reset_date = datetime.combine(
                current_monday, datetime.min.time()
            ).replace(tzinfo=CST)
            self._daily_limit_hit_count_this_week = 0
            if prev_pnl != 0.0:
                logger.info(f"📅 Weekly P&L reset (prev week: ${prev_pnl:+.2f})")

    def record_realized_pnl(self, pnl: float, now: Optional[datetime] = None) -> None:
        """Record realized P&L for weekly tracking. Called after each trade close."""
        self._ensure_weekly_reset(now)
        self._weekly_realized_pnl += pnl
        logger.info(f"📊 Weekly realized P&L: ${self._weekly_realized_pnl:+.2f} (trade: ${pnl:+.2f})")

    def record_daily_limit_hit(self) -> None:
        """Record that the daily loss limit was hit (for kill-switch logic)."""
        self._daily_limit_hit_count_this_week += 1
        if self._daily_limit_hit_count_this_week >= 3:
            self.activate_kill_switch(reason="DAILY_LIMIT_HIT_3X_IN_WEEK")
    ...
    # ------------------------------------------------------------------
    # FEB 2026: Kill-switch with auto-reset
    # ------------------------------------------------------------------

    def activate_kill_switch(self, reason: str = "MANUAL", manual: bool = False) -> None:
        """Activate kill-switch. Manual kills require manual reset."""
        self._kill_switch_active = True
        self._kill_switch_manual = manual or reason == "MANUAL"
        self._kill_switch_trigger_ts = now_cst()
        logger.warning(f"🚫 KILL SWITCH ACTIVATED: {reason} (manual={self._kill_switch_manual})")

    def deactivate_kill_switch(self) -> None:
        """Manually deactivate kill-switch."""
        self._kill_switch_active = False
        self._kill_switch_manual = False
        self._kill_switch_trigger_ts = None
        logger.info("✅ Kill switch deactivated (manual reset)")

    def _check_kill_switch_auto_reset(self, now: Optional[datetime] = None) -> None:
        """Auto-reset kill-switch if conditions are met.

        Auto-reset requires ALL:
        1. Kill-switch was not manually activated
        2. At least 1 calendar day has passed since trigger
        3. New trading day (after 08:30 CT / 09:30 ET)
        4. Equity is recovering (last equity > 0, not in peak drawdown)
        """
        if not self._kill_switch_active:
            return
        if self._kill_switch_manual:
            return  # Manual kills require manual reset
        if self._kill_switch_trigger_ts is None:
            return
        ...
        # At least 1 calendar day since trigger
        trigger_cst = self._kill_switch_trigger_ts
        try:
            trigger_cst = trigger_cst.astimezone(CST) if trigger_cst.tzinfo else trigger_cst
        except Exception:
            return
        if (cst_now.date() - trigger_cst.date()).days < 1:
            return

        # Must be during trading hours (after 08:30 CT)
        from datetime import time as dt_time
        if cst_now.time() < dt_time(8, 30):
            return

        # Must not be in peak drawdown
        if self._drawdown_active:
            return

        # All conditions met — auto-reset
        logger.info(
            f"✅ Kill switch auto-reset: triggered {trigger_cst.strftime('%Y-%m-%d %H:%M')} CT, "
            f"now {cst_now.strftime('%Y-%m-%d %H:%M')} CT"
        )
        self._kill_switch_active = False
        self._kill_switch_trigger_ts = None
    ...
    def evaluate_entry(
        ...
        # 0.25) Kill-switch check (check auto-reset first)
        self._check_kill_switch_auto_reset(now)
        if self._kill_switch_active:
            reason = "KILL_SWITCH_ACTIVE"
            logger.warning("🚫 RiskGate block: Kill switch is active")
            return RiskGateResult(False, reason, levels)

        # 0.4) Weekly loss limit
        self._ensure_weekly_reset(now)
        if self.config.weekly_max_loss_usd > 0:
            if self._weekly_realized_pnl <= -abs(self.config.weekly_max_loss_usd):
                reason = f"WEEKLY_LOSS_LIMIT:${self._weekly_realized_pnl:.0f}<=-${abs(self.config.weekly_max_loss_usd):.0f}"
                logger.warning(f"🚫 RiskGate block: {reason}")
                return RiskGateResult(False, reason, levels)
