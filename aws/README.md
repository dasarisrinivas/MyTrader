...
| `rag.min_weighted_win_rate_soft_floor` | Lower bound that activates when the retrieval set is thin. Set it below `min_weighted_win_rate` to relax the regime filter until enough similar trades accumulate. | `0.5` (override with `MIN_WEIGHTED_WIN_RATE_SOFT_FLOOR`) |
| `rag.min_similar_trades_for_full_threshold` | Number of matches required before the strict win-rate threshold is enforced again. Leave at `0` to disable the relaxed behavior entirely. | `0` (override with `MIN_SIMILAR_TRADES_FOR_FULL_THRESHOLD`) |
