...
class OrderTracker:
    ...
    def _format_order_placement_callsite() -> str:
        ...
        Gated behind env var MYTRADER_ORDER_TRACKER_CALLSITE to avoid overhead/noise.
        """

        if os.getenv("MYTRADER_ORDER_TRACKER_CALLSITE", "").strip().lower() not in {"1", "true", "yes"}:
            return ""
        try:
            stack = inspect.stack()
