...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    metrics.add_metric(name="ResponseTime", unit=MetricUnit.Milliseconds, value=duration_ms)
    metrics.add_metric(name="CacheHits", unit=MetricUnit.Count, value=1 if event.get('cached') else 0)
    confidence_val = response.get('confidence', merged_event.get('confidence', 0))
    metrics.add_metric(name="ConfidenceScore", unit=MetricUnit.Percent, value=float(confidence_val or 0) * 100)
    return response
